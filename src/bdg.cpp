...
void cBdG_Bulk::update(int nk, double kx, double ky){
  ...
  if (nk == -1) {
	...
	for (int it = 0; it < lenT; ++it) {
	  Gamma2 += complex<double>(DELTA_r_t[it],-DELTA_i_t[it])*exp(_myI*2*M_PI*(q-p)/_T*time_t[it])/_T*dt;
	}
